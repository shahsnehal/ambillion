...
function* handleFetchProductDetails(action: {
    ...
    try {
        ...
        yield put({ type: GET_PRODUCTDETAILS_SUCCESS, payload: response.data });
    } catch (error: unknown) {
        let errorMessage = 'An unknown error occurred';
        if (error instanceof Error) {
            errorMessage = error.message;
        }
        yield put({ type: GET_PRODUCTDETAILS_FAILURE, error: errorMessage });
    }
}
...
export default function* productSaga() {
    ...
    yield takeLatest(GET_PRODUCTDETAILS_REQUEST, handleFetchProductDetails);
}
